#############################################################################################
# Zipcode-year level final data combination 
# last modified: 08/07/24
#############################################################################################
...
# cleanups by zip x year
load('data/processed/00_data_intermediate/00_data_zip_year.rda')
...
data <- data %>% distinct(zip, year, .keep_all = T)

# drop any CA (before 2016) or treated NY 
data <- data %>% filter(state != "CA") %>% filter(!(state == "NY" & type == "control"))

# balanced panel 
dataBalanced <- data %>% filter(year >= 2016 & year <= 2023) %>% mutate(ind = 1) %>% group_by(zip) %>% mutate(sum = sum(ind)) %>% filter(sum == 8)

# Save --------------------------------------------------------------------------------------

save(data, file="data/processed/02_merged_zip_year.rda")
save(dataBalanced, file="data/processed/02_merged_zip_year_balanced.rda")
