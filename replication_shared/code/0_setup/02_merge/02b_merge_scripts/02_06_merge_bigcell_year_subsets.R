#############################################################################################
# Bigcell-year level final data combination 
...
file_suffixes <- c("coast", "river", "river_10km", "lake", "lake_10km", "other", "drainage", "drainage_nolake", "10km", "iccd", "big", "small", "nokids")

# Function to process single dataset -------------------------------------------------------- 

process_dataset <- function(suffix) {
  ...
  # cleanups by cell x year
  load(paste0('data/processed/00_data_intermediate/00_data_bigcell_year_', suffix, '.rda'))
  ...
  data <- data %>% arrange(groupID1, year)
  ...
  data <- data %>% mutate(treatedAny = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect, 1, 0), 
                          ...
                          treatedSpillover1 =  ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > neighborFirstEffect & neighborInd == 1, 1, 0),
                          treatedSpillover2 =  ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > neighborFirstEffect & neighborInd == 2, 1, 0),
                          treatedSpillover3 =  ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > neighborFirstEffect & neighborInd == 3, 1, 0))
  ...
  # balanced panel 
  dataBalanced <- data %>% filter(year >= 2016 & year <= 2023) %>% mutate(ind = 1) %>% group_by(groupID1) %>% mutate(sum = sum(ind)) %>% filter(sum == 8) 
  
  # Save ---
  
  # if the suffix is in a certain group
  if(suffix %in% c("coast", "river", "river_10km", "lake", "lake_10km")){
    save(data, file=paste0("data/processed/02_merged_bigcell_year_", suffix, ".rda"))
    save(dataBalanced, file=paste0("data/processed/appendix/02_merged_bigcell_year_balanced_", suffix, ".rda"))
  } else{
    save(data, file=paste0("data/processed/appendix/02_data_merged/02_merged_bigcell_year_", suffix, ".rda"))
    save(dataBalanced, file=paste0("data/processed/appendix/02_data_merged/02_merged_bigcell_year_balanced_", suffix, ".rda")) 
  }
...
for(suffix in file_suffixes) {
  message(paste("Processing dataset with suffix:", suffix))
  process_dataset(suffix)
}
