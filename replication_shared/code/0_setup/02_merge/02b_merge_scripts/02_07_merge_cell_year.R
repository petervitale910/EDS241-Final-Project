#############################################################################################
# Cell-year level final data combination 
...
load('data/processed/00_data_intermediate/00_data_cell_year.rda')
...
data <- data %>% arrange(groupID01, year)
...
# balanced panel 
dataBalanced <- data %>% filter(year >= 2016 & year <= 2023) %>% mutate(ind = 1) %>% group_by(groupID01) %>% mutate(sum = sum(ind)) %>% filter(sum == 8)  

# Save --------------------------------------------------------------------------------------

save(data, file="data/processed/appendix/02_data_merged/02_merged_cell_year.rda")
save(dataBalanced, file="data/processed/appendix/02_data_merged/02_merged_cell_year_balanced.rda")
